...
#define  insque(inp, head) \
    tn_np_que_insert_entry((struct tn_np_que *) (inp), \
                                      (struct tn_np_que *) (head))

#define  remque(inp) \
     tn_np_que_remove_entry((struct tn_np_que *) inp)
...
//int tn_atomic_set(int * var_to_set, int value);
